...
async function usersSearch(q){
  if(!q||!q.trim())return []
  const url=SB_REST+'/users?select=name,tool,ew&name=ilike.*'+encodeURIComponent(q.trim())+'*&order=name.asc&limit=8'
  try{
...
async function usersUpsert(name,tool,ew){
  if(!name||!name.trim())return
  try{
    await fetch(SB_REST+'/users',{
      method:'POST',
      headers:{...SB_HEADERS,'Prefer':'resolution=merge-duplicates,return=minimal'},
      body:JSON.stringify({name:name.trim(),tool:tool||null,ew:!!ew,updated_at:new Date().toISOString()})
    })
